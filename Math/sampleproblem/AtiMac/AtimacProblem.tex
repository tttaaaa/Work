...
Atiyah Macdonaldの問題の解答をUpしておく．

\section{RINGS AND IDEALS}
\label{sec:RINGS AND IDEALS}
\begin{exs}
 Let $x$ be a nilpotent element of a ring $A$. Show that $1 + x $ is a unit of $A$.
 Deduce that the sum of a nilpotent element and a unit is a unit
\end{exs}
\begin{proof}
 $x$がnilpotentなので，$x^m=0$となる.これより
\begin{equation*}
  (1 + x )(\sum_{i=0}^{m-1}(-x)^i) = 1 +(-x)^m=1
\end{equation*}
となるため，$1+x$が単元であることがわかる．
unitとnilpotentの和は，unitとnilpotentの積がnilpotentになることから，
unitをかけて，1とnilpotentの和となり，特にunitにできることから言える．
\end{proof}

\section{MODULES}
\label{sec:MODULES}

\section{RINGS ANS FRACTION OF MODULES}
\label{sec:RINGS ANS FRACTION OF MODULES}

\section{PRIMAY DECOMPOSITION}
\label{sec:PRIMAY DECOMPOSITION}

\section{Integral Dependence and Valuations}
\label{sec:Integral Dependence and Valuations}
\begin{exs}
 Lef $f:A \to B$ be an itengral homomorphism of rings. Show that $f^*: \mathrm{Spec}B \to \mathrm{Spec}A$ is a
 \it{closed} mapping,i.e. that it maps closed sets to closed sets.
\end{exs}
\begin{proof}
